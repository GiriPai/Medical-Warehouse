...
// @desc    Update a hospital by admin except avatar upload
// @access  Private (admin)
router.put(
  "/:id",
  [
  ...
  async (req, res) => {
    ...
    try {
      let hospital = await Hospital.findOne({ _id: req.params.id });

      if (!hospital) {
        return res
          .status(400)
          .send({ errors: [{ msg: "Hospital Doesn't Exists" }] });
      }
      ...
      if (
        ...
        hospital.registerNumber = registerNumber;
      if (name && name !== null && name !== undefined) hospital.name = name;
